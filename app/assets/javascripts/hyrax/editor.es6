...
export default class {
  ...
  constructor(element) {
    ...
    this.adminSetWidget = new AdminSetWidget(
      element.find('select[id$="_admin_set_id"]')
    );
  ...
  authoritySelect() {
    $('[data-authority-select]').each(function() {
      ...
      let usesControlledVocab = $(this).data().usesControlledVocab;
      let options = {
        selectBox: 'select.' + authoritySelect,
        inputField: 'input.' + authoritySelect,
        usesControlledVocab: usesControlledVocab
      };
  ...
  controlledVocabularies() {
    this.element
      .find('.controlled_vocabulary.form-group')
      .each(
        (_idx, controlled_field) =>
  ...
  sharingTabVisiblity(visible) {
    if (visible) this.sharingTabElement.removeClass('hidden');
    else this.sharingTabElement.addClass('hidden');
  }

  relationshipsControl() {
    let collections = this.element.find(
      '[data-behavior="collection-relationships"]'
    );
    collections.each((_idx, element) =>
      new RelationshipsControl(
        element,
  ...
  saveWorkControl() {
    new SaveWorkControl(
      this.element.find('#form-progress'),
      this.adminSetWidget
    );
  }
