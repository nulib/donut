  ...
  class MigrationService
      ...
      {}.tap do |descriptive_metadata|
        ...
        descriptive_metadata['creator'] =
          %w[creator nul_creator].flat_map do |field|
            image.attributes.fetch(field).map do |value|
              value.is_a?(String) ? authority_map.to_id(value) : value.id
            end.map { |id| { term: { id: id } } }
          end.compact
    ...
    def convert_coded_term_mapping(mapping, image)
      mapping.keys.flat_map do |field|
        image.attributes.fetch(field).map do |value|
          value.is_a?(String) ? authority_map.to_id(value) : value.id
        end.map { |id| mapping.fetch(field).merge(term: { id: id }) }
      end
    end
